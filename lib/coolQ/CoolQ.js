...
class CoolQ {
  ...
  async sendJiziOneItemMessage(item, args) {
    const {
      ...
      endTime,
      res1,
      pk0Donation, pk1Donation, pk2Donation, pk3Donation, pk4Donation // TODO: pk逻辑，用完后删除
    } = args;
    ...
    const tmp = `${ taoBaConfig.template }
当前PK情况：
袁雨桢 ¥${ pk0Donation }
彭嘉敏 ¥${ pk1Donation }
张茜 ¥${ pk2Donation }
...
刘胜男 ¥${ pk4Donation }`;
    ...
    const msg = renderString(tmp/* taoBaConfig.template */, {
      nickname: item.nick,
  ...
  async sendJiziMessage(result) {
    ...
    // TODO: pk逻辑，用完后删除
    const [pk0, pk1, pk2, pk3, pk4] = await Promise.all([
      requestDetail('7630'),
      requestDetail('7631'),
      requestDetail('7633'),
      requestDetail('7634'),
    ...
    const [pk0Donation, pk1Donation, pk2Donation, pk3Donation, pk4Donation] = [
      pk0.data.datas.donation,
      pk1.data.datas.donation,
      pk2.data.datas.donation,
      pk3.data.datas.donation,
      pk4.data.datas.donation
    ];
